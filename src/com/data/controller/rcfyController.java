...
import com.data.service.rcfyService;
...
@Scope("prototype")
@Controller
@RequestMapping("/rcfyController")
public class rcfyController extends BaseManageController{
	
	@Resource
	private rcfyService rcfyService;
	/**
	 * 入口
	 */
	...
	public Map<String, Object> selectUserInfoList(UserInfoEntity userInfoEntity) throws IOException {
		...
		map.put("total", rcfyService.countUserInfo(userInfoEntity));
		map.put("rows", rcfyService.listUserInfo(userInfoEntity));
		return map;
	}
	...
	public String openEditPage(ModelMap modelMap,String id) {
		modelMap.put("userInfo", rcfyService.getUserInfoById(id));
		return "rcfy/userInfoEdit";
	}
	...
	public int insertUserInfo(UserInfoEntity userInfoEntity) throws IOException {
		...
		return rcfyService.insertUserInfo(userInfoEntity);
	}
	...
	public long updateUserInfo(UserInfoEntity userInfoEntity) throws IOException {
		SessionManageEntity user = SessionUtil.getSession(request);
		userInfoEntity.setUpdateBy(user.getUserId());
		return rcfyService.updateUserInfo(userInfoEntity);
	}
	...
	public long deleteUserInfo(String ids) throws IOException {
		return rcfyService.deleteUserInfo(ids);
	}
