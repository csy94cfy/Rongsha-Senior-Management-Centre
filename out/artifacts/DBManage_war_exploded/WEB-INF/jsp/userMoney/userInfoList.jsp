...
// 		$("#searchFlag").comboBox();
		...
		var toolbar = [
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'850px',height:'450px', title : '新增对口信息', url : 'userMoneyController/openEditPage' })
				} 
			},
			{text : '修改',validCount : '1',handler : function(rows){
				modal.open({width:'850px',height:'450px',title:'编辑对口信息',url:'userMoneyController/openEditPage?id='+rows[0].id})
			}},
			{text : '删除',validCount : '1+',handler : function(rows){
				alertify.confirm("确认删除吗?", function(e){
					...
					$.ajax({
						url: 'userMoneyController/deleteUserInfo',
						type: 'post',//提交的方式
						dataType:'json',
		...
		//列表方法
		$('#table').datagrid({
			url : 'userMoneyController/selectUserInfoList',//查询数据的地址
			form : 'selectform',//用于筛选的form id
			//字段
			columns : [
				{field : 'realName',title : '工作人员姓名',width : 150},
				{field : 'userPhone',title : '对口老人',width : 130},
				{field : 'openid',title : '对口情况',width : 170}
			],
			//按钮功能栏
			toolbar : toolbar,
				...
				<input id="queryCondition"type="hidden" name="queryCondition" >
					<div class="widget-screen">
						...
						<div class="widget-item">
							<input class="form-control" name=realName type="text" placeholder="姓名模糊查询">
						</div>
					</div>
