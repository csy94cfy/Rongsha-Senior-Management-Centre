...
// 		$("#searchFlag").comboBox();
		...
		var toolbar = [
			...
			handler : function(rows) {
				modal.open({width:'850px',height:'450px', title : '添加公告信息', url : 'ggxxController/openEditPage' })
			} 
		},
			{text : '修改',validCount : '1',handler : function(rows){
				modal.open({width:'850px',height:'450px',title:'编辑公告信息',url:'ggxxController/openEditPage?id='+rows[0].id})
			}},
			{text : '删除',validCount : '1+',handler : function(rows){
				alertify.confirm("确认删除吗?", function(e){
					...
					$.ajax({
						url: 'ggxxController/deleteUserInfo',
						type: 'post',//提交的方式
						dataType:'json',
		...
		//列表方法
		$('#table').datagrid({
			url : 'ggxxController/selectUserInfoList',//查询数据的地址
			form : 'selectform',//用于筛选的form id
			//字段
			columns : [
				{field : 'realName',title : '公告内容',width : 150},
				{field : 'userPhone',title : '公告详细',width : 130},
				{field : 'openid',title : '公告类型',width : 170}
			],
			//按钮功能栏
			toolbar : toolbar,
			...
			<div class="widget-control">
				...
				<input id="queryCondition"type="hidden" name="queryCondition" >
					<div class="widget-screen">
						<label for="realName" class="control-label">公告内容</label>
						<div class="widget-item">
							<input class="form-control" name=realName type="text"  >
						</div>
					</div>
					<div class="widget-screen">
						<label for="userPhone" class="control-label">公告详细</label>
						<div class="widget-item">
							<input class="form-control" name="userPhone" type="text" >
						</div>
					</div>
					<div class="widget-screen">
						<label for="openid" class="control-label">公告类型</label>
						<div class="widget-item">
