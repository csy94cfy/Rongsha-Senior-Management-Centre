...
// 		$("#searchFlag").comboBox();
		...
		var toolbar = [
			...
			handler : function(rows) {
				modal.open({width:'850px',height:'450px', title : '添加需求信息', url : 'xqxxController/openEditPage' })
			} 
		},
			{text : '修改',validCount : '1',handler : function(rows){
				modal.open({width:'850px',height:'450px',title:'编辑需求信息',url:'xqxxController/openEditPage?id='+rows[0].id})
			}},
			{text : '删除',validCount : '1+',handler : function(rows){
				alertify.confirm("确认删除吗?", function(e){
					...
					$.ajax({
						url: 'xqxxController/deleteUserInfo',
						type: 'post',//提交的方式
						dataType:'json',
		...
		//列表方法
		$('#table').datagrid({
			url : 'xqxxController/selectUserInfoList',//查询数据的地址
			form : 'selectform',//用于筛选的form id
			//字段
			columns : [
				{field : 'realName',title : '需求名称',width : 150},
				{field : 'userPhone',title : '需求详细',width : 130},
				{field : 'openid',title : '需求类型',width : 170}
			],
			//按钮功能栏
			toolbar : toolbar,
			...
			<div class="widget-control">
				...
				<input id="queryCondition"type="hidden" name="queryCondition" >
					<div class="widget-screen">
						<label for="realName" class="control-label">需求名称</label>
						<div class="widget-item">
							<input class="form-control" name=realName type="text"  >
						</div>
					</div>
					<div class="widget-screen">
						<label for="userPhone" class="control-label">需求详细</label>
						<div class="widget-item">
							<input class="form-control" name="userPhone" type="text"  >
						</div>
					</div>
					<div class="widget-screen">
						<label for="openid" class="control-label">需求类型</label>
						<div class="widget-item">
